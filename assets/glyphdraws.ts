...
flower(s){const r=11,c=50,lim=2*r*1.04,id="fc"+Math.random().toString(36).slice(2);const d=add(s,"defs",{});const cp=add(d,"clipPath",{id});add(cp,"circle",{cx:c,cy:c,r:2*r});const g=add(s,"g",{"clip-path":"url(#"+id+")"});for(let i=-3;i<=3;i++)for(let j=-3;j<=3;j++){const x=i*r+j*r/2,y=j*r*R3/2;if(Math.hypot(x,y)<=lim)ring(g,c+x,c+y,r,1,.85);}ring(s,c,c,2*r,1.4,1);},
...
sriyantra(s){const c=50,up=[[78,40],[68,32],[60,24],[54,17]],dn=[[18,42],[30,34],[42,26],[50,18]];const tri=(ay,hw,d)=>{const by=d?ay-46:ay+46;return[[c,ay],[c-hw,by],[c+hw,by]];};up.forEach(a=>P(s,pth(tri(a[0],a[1],false),true),.8,"none",.85));dn.forEach(a=>P(s,pth(tri(a[0],a[1],true),true),.8,"none",.85));dot(s,c,50,2);add(s,"rect",{x:c-44,y:6,width:88,height:88,fill:"none",stroke:K,"stroke-width":.6,opacity:.4});},
...
goldenspiral(s){let b=.306,d="",c=50;for(let t=0;t<6.3*Math.PI;t+=.12){const rr=2.4*Math.exp(b*t),x=c+rr*Math.cos(t),y=c+rr*Math.sin(t);d+=(d?"L":"M")+x.toFixed(1)+" "+y.toFixed(1);}P(s,d,1.4,"none",.95);},
...
ouroboros(s){const c=50,r=30;P(s,"M "+(c+Math.cos(-1)*r)+" "+(c+Math.sin(-1)*r)+" A "+r+" "+r+" 0 1 1 "+(c+Math.cos(-.2)*r)+" "+(c+Math.sin(-.2)*r),3.2,"none",.9);const hx=c+Math.cos(-.2)*r,hy=c+Math.sin(-.2)*r;P(s,"M "+(hx-5)+" "+(hy-6)+" L "+(hx+6)+" "+hy+" L "+(hx-5)+" "+(hy+6)+"Z",1,K,.95);},
